...
# OrderService는 DB 기반으로 구현되었다고 가정합니다.
...
class ItemService:
    ...
    def load(cls):
        ...
        try:
            with conn.cursor() as cursor:
                ...
                print(f"시스템: 현재 등록된 상품 수는 {count}개 입니다.")
        finally:
            conn.close()
    ...
    def delete_item(cls):
        ...
        try:
            ...
            # 외래키 제약조건(이미 주문된 상품 등) 때문에 삭제가 안 될 수 있음
        ...
        try:
            with conn.cursor() as cursor:
                ...
                for cart in Session.cart:
                    ...
                    # 주문 내역 추가 (OrderService 호출 또는 직접 SQL)
    ...
    def admin_menu(cls):
        ...
        while True:
            ...
            print("1. 상품 리스트")
            print("2. 새 상품 등록")
            print("3. 상품 정보 수정 (이름/가격/카테고리)")
            print("4. 재고 수량 관리 (입고/조정)")
            print("5. 상품 삭제")
            print("6. 전체 판매 내역 보기 (Orders)")
            print("7. 환불 요청 승인 처리")
            print("0. 이전 메뉴로")
    ...
    def purchase(cls):
        ...
        try:
            with conn.cursor() as cursor:
                # 1. orders 테이블에 마스터 레코드 생성 (누가, 총 얼마)
                ...
                # 2. 장바구니 상품들을 order_items에 하나씩 저장
                for cart in Session.cart:
                    item = cart["item"]
                    qty = cart["qty"]
    ...
    def run(cls):
        ...
        while True:
            print("\n" + "=" * 25 + "\n  상품 관리 시스템\n" + "=" * 25)
            print("1. 상품 목록 및 장바구니")
            print("2. 장바구니 보기")
            print("3. 결제하기")
            print("4. 구매 내역 및 취소/환불")  # 추가된 메뉴
